...
def process_master_csv(master_file_path, low_stock_threshold, selected_product_types, selected_brands, sort_by_amount):
    try:
        ...
        # 1. SANITIZE DATA: Convert columns to uppercase to avoid case-sensitivity
        master_df['Brand'] = master_df['Brand'].astype(str).str.strip().str.upper()
        master_df['Category'] = master_df['Category'].astype(str).str.strip().str.upper()
        master_df['Room'] = master_df['Room'].astype(str).str.strip().str.upper()

        selected_product_types = [pt.upper() for pt in selected_product_types]
        selected_brands = [b.upper() for b in selected_brands]

        # 2. SMART FILTER: Map basic categories to Dutchie's complicated categories
        keyword_mapping = {
            "FLOWER": ["FLOWER"],
            "EDIBLE": ["GUMMIES", "CHOCOLATE", "CANDY", "BAKED", "BEVERAGE", "EDIBLE"],
            "PREROLL": ["PREROLL", "BLUNT"],
            "CARTRIDGE": ["CARTRIDGE", "AIO", "POD", "VAPE"],
            "EXTRACT": ["RESIN", "ROSIN", "SHATTER", "RSO", "EXTRACT", "WAX", "BADDER", "DIAMOND"],
            "TOPICAL": ["TOPICAL", "LOTION", "BALM"],
            "MERCH": ["CLOTHING", "ACCESSORIES", "BATTERY", "MERCH", "RAW MATERIAL"]
        }

        # Build a list of keywords to search for based on the boxes checked
        search_keywords = []
        for pt in selected_product_types:
            if pt in keyword_mapping:
                search_keywords.extend(keyword_mapping[pt])
            else:
                search_keywords.append(pt)

        # Filter the DataFrame if any keywords exist in the Dutchie 'Category' column
        if search_keywords:
            pattern = '|'.join(search_keywords)
            master_df = master_df[master_df['Category'].str.contains(pattern, case=False, na=False)]

        # 3. Filter by selected brands if any are selected
        if selected_brands:
            master_df = master_df[master_df['Brand'].isin(selected_brands)]

        # 4. Filter for Front of House and Back of House 
        front_of_house_df = master_df[master_df['Room'].isin(['SALES FLOOR', 'DISPLAY CASE'])]
        ...
        if sort_by_amount and not available_in_back.empty:
...
def process_csv_files(front_of_house_df, back_of_house_df, low_stock_threshold):
    try:
        ...
        # Aggregate front of house data by Brand and Online title (Dutchie uses 'Online title')
        front_of_house_aggregated = front_of_house_df.groupby(['Brand', 'Online title'])['Available'].sum().reset_index()

        # Aggregate back of house data by Brand and Online title
        back_of_house_aggregated = back_of_house_df.groupby(['Brand', 'Online title'])['Available'].sum().reset_index()

        # Merge front of house and back of house data
        merged_data = pd.merge(back_of_house_aggregated, front_of_house_aggregated, on=['Brand', 'Online title'], how='left', suffixes=('_back', '_front'))
        ...
        available_in_back = available_in_back.rename(columns={
            'Online title': 'Product Name', 
            'Available_front': 'Available Front', 
            'Available_back': 'Available Back'
        })

        # Reorder columns
        if not available_in_back.empty:
...
def run_processing():
    ...
    try:
        ...
        if low_stock_items_df.empty:
            report_text.insert(tk.END, "No items found matching your criteria. Everything looks stocked up!")
            messagebox.showinfo("Empty Report", "No low stock items matched your filters. No Excel file was generated.")
            return
...
# Setup GUI
root = tk.Tk()
root.title("What's Missing - Dutchie Edition")
...
# YOUR FAMILIAR CATEGORIES ARE BACK!
product_types = ["FLOWER", "EDIBLE", "PREROLL", "CARTRIDGE", "EXTRACT", "TOPICAL", "MERCH"]

product_type_vars = {product_type: tk.BooleanVar() for product_type in product_types}

product_type_label = tk.Label(product_type_frame, text="Select Product Categories")
...
select_all_product_types_button = tk.Button(product_type_frame, text="Select All Categories", command=select_all_product_types)
select_all_product_types_button.pack()
deselect_all_product_types_button = tk.Button(product_type_frame, text="Deselect All Categories", command=deselect_all_product_types)
